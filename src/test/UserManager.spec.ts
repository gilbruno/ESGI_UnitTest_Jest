...
describe('UserManager', () => {
    ...
    test('throws error when adding a user with an existing ID', () => {
        userManager.addUser(mockUser)
        expect(() => userManager.addUser(mockUser)).toThrow("User ID already exists")
    })

    test('throws error when adding a user with an existing email', () => {
        userManager.addUser(mockUser)
        expect(() => userManager.addUser({ ...mockUser2, email: 'john@example.com' })).toThrow("User email already exists")
    })
    ...
    test('updates a user', () => {
        ...
        expect(userManager.findUserById(mockUser.id)).toEqual({ ...mockUser, name: 'Jane Doe' })
    })
    ...
    test('throws error when updating a non-existent user', () => {
        expect(() => userManager.updateUser(99, { name: 'Unknown' })).toThrow("User not found")
    })
